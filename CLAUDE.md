...
**Phases 0 through 3 are built, merged, and committed; phases 4, 5 and 6 landed on top, and all three are now live-verified. Phase 7 (workspace IDE) is built and suite-green but not yet browser-verified.** 296 root tests + 361 client tests. Phase 3 was the last *planned* phase. **The MVP queue that used to sit here — deploy, `POST /api/rooms` hardening, the two named test gaps, the Day 5 demo — is now closed out**, per the user directly: the app is deployed and live at `https://nexus-mvp.fly.dev/`, redeployed after the hardening fix below; the demo has been run; and phase 4 has had its own live two-browser pass.
...
- **Phase 7 (post-MVP)** — the workspace IDE. The room stops being a chat transcript and becomes an editor surface: a jailed read-only workspace API (`src/server/workspace.ts` — `realpathSync` on **both** candidate and root, because a hostile cloned repo can commit a symlink out of the tree and a lexical `resolve()` check cannot see it), a debounced fs watcher (`watcher.ts`), git status/diff reusing `publish.ts`'s `GitRunner` seam (`gitStatus.ts`), five REST routes, in-session model switching via `setModel` (I1 holds — it mutates the running session, never a second `query()`), and context-window telemetry. Client side: a file tree, Shiki-highlighted viewer in a lazily-loaded chunk, **rendered diffs**, and a prompt dock. `SideRail` is gone, its three sections rehomed. Plans: `docs/plans/phase-7{a,b,c}-*.md`; `phase-7-workspace-ide.md` is the architecture record and contains six defects found by compiling it — **do not code from it**. **Suite-green and mutation-tested (5/5 killed), but NOT browser-verified and never driven by a live agent** — the governance moment (a non-driver approving a rendered diff) is the point of the phase and is unobserved. See the newest `sessions/` folder.
...
npm test             # vitest run — 296 tests today
npm run test:client  # npm --prefix client test — 361 tests
