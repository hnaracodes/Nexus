...
# create the node_modules/@syncode/protocol symlink — the install would succeed
...
COPY apps ./apps
# @syncode/protocol first, explicitly. Both the server build and the client build
...
# filtered `--workspace=` install risks the @syncode/protocol symlink not being
...
# and checked the exact thing the warning named — `node_modules/@syncode/protocol`
...
# `--workspace @syncode/protocol` is what keeps the symlink real.
RUN npm ci --omit=dev --ignore-scripts \
  --workspace @syncode/server --workspace @syncode/protocol --include-workspace-root \
...
# The @syncode/protocol symlink installed above points here. Without this it dangles.
